...
- **경계선**: `js/map-lines.js` — `tools/make-lines.py` 로 생성(직접 고치지 말 것).
  ⚠️ **도형(`path.land`/`path.sggp`)에 절대 흰 외곽선을 주지 말 것.**
  도형마다 외곽선을 그리면 이웃과 맞닿은 경계가 두 번 그려지는데,
  두 도형의 좌표가 서로 달라(정확히 겹치는 선분은 30%뿐) **나란한 두 줄**로 보인다.
  그래서 중복 선분을 걷어낸 한 겹짜리 경계망(`SIDO_LINE`/`SGG_LINE`)을 따로 얹는다.
  도형에는 **제 fill 과 같은 색의 얇은 선**만 둘러 도형 사이 틈을 메운다.
  시군구 선은 z3(폭<95)부터 나타난다. 동 경계는 z5 에서만 흐린 점선.

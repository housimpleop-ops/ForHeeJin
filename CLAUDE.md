...
- **경계선**: `js/map-lines.js` — `tools/make-lines.py` 로 생성(직접 고치지 말 것).
  ...
  ⚠️ **④ 어두운 모드에서 도형마다 `filter` 를 걸면 안 된다.** `saturate/brightness` 를
  249개 `.sggp` 에 각각 걸면 브라우저가 도형마다 따로 합성해 **경계마다 얇은 이음매**가
  생긴다. 지도 전체에 그물 같은 얼룩으로 보인다. 어두운 색을 미리 계산해 `fill` 로 박을 것.

  ⚠️ **⑥ 조사팀끼리 말이 갈리면 카카오 등재 여부로 판정한다.**
  드래곤힐스파·실로암사우나·미란다 스파플러스가 팀마다 "운영 중" / "폐업"으로 엇갈렸는데,
  카카오에 업소가 없으면(주차장 항목만 있거나 동명 다른 업소만 나오면) 제외했다.
  여행사 사이트에 이용권이 남아 있어도 실제로는 문 닫은 곳이 있다.

  ⚠️ **⑤ 이웃 도형끼리 어긋난 것은 `SGG_FIT` 로 해결한다.** 같은 경계를 두 시군구가
  각자 조금씩 다르게 그려(최대 0.5단위) 한쪽 색이 남의 땅으로 넘어간다.
  `snap_shapes()` 가 모든 점을 대표 경계선에 투영해 두 도형이 같은 선을 공유하게 만든다.
  **칠은 반드시 `SGG_FIT` 를 쓸 것**(`SGG_SHAPES` 는 클릭 판정·라벨용).
  ※ 어긋남을 잴 때 "가장 가까운 다른 도형까지의 거리"로 재면 **해안선 때문에 값이 부풀려진다**.
  실제로 맞닿은 경계만 봐야 한다.
...
- `bookLinks(s)` 가 목록 상세 맨 위에 버튼 줄을 만든다 — 「🔍 검색」「🗺️ 지도·전화」.
  `BOOK_HINT[cat]` 이 분야에 맞는 검색어를 붙인다(숙박 "예약", 공연 "예매", 스키 "리프트권 예약").
- `book` 값이 `http` 로 시작하면 「🎟️ 공식 예약」 버튼이 맨 앞에 붙고 정보 줄도 링크가 된다.
- ⚠️ **여기어때·야놀자·캠핏 같은 예약 사이트의 검색 URL 을 직접 만들지 말 것.**
  주소 형식이 자주 바뀌어 깨진 링크가 된다. 네이버 검색·카카오맵만 안정적이다.
- 조사 에이전트에게 `book` 을 필수로 요구할 것 — 공식 URL 이 없으면
  `캠핏`·`숲나들e`·`전화 0xx-xxx-xxxx`·`예약 불필요 (현장 입장)` 처럼 어디서 예약하는지 적게 한다.
...
## 스팟 데이터 (가볼 곳 모음)
- `js/data-run.js` — 러닝 188곳(RUN_SPOTS). 러닝 전용 항목(km·노면·경사·그늘·벌레) 때문에 파일 분리 유지
...
### 현재 수량 (2026-08-27)
전시·구경 208 / 등산 155 / 숙박 146 / 카페 128 / 맛집 128 / 쇼핑·시장 104 / 축제 99 /
캠핑 98 / 계곡 91 / 드라이브 91 / 온천 87 / 해변 86 / 공연 82 / 스키·썰매 52
= **스팟 1,555곳** + 러닝 188곳 = **1,743곳**, 좌표 100%
- **그림지도의 시군구 250여 칸에 빈칸이 하나도 없다.** 예전엔 양구·영양·고령·사천·의령·함안·
  마산합포·마산회원이 통째로 비어 있었고, 철원·예천은 이웃 시군 스팟이 경계상자에 걸려
  있을 뿐 자기 지역 스팟이 0곳이었다. 빈칸을 세는 법은 아래 '빈 시군구 세는 법' 참고
- **예약 정보 1,023곳, 그중 공식 예약 URL 424곳.** 링크는 전부 눌러서 200 응답을 확인했다
  (하얏트·메리어트·반얀트리·인천 남동구청은 봇 차단으로 403이 뜨지만 브라우저에서는 정상)
- **축제 99곳은 전수 완료.** 온천 87곳 중 83곳 — 나머지 4곳은 걸 만한 주소가 실제로 없거나
  (스파렉스 동묘·온양온천 족욕탕) 휴업 중이라(진안홍삼스파·스파밸리) 일부러 비워 뒀다
- 등산·카페·해변·계곡·드라이브·쇼핑에 예약칸이 비어 있는 건 **정상**이다. 예약이 필요 없는 곳들이다
- **해변은 `acts4` 배열로 4가지 활동을 겹쳐 표시**한다(해수욕 72·서핑 17·프리다이빙 4·
  스쿠버 5·해당없음 14). 해변을 고르면 세부 칩이 이 4가지로 바뀌고, 한 해변이 여러 칸에
  동시에 들어간다. 서핑·다이빙이 안 되는 곳은 그 줄을 감춘다(BEACH_ACTS 참고).
  `acts4` 가 없으면 세부 필터에서 통째로 사라지므로 정규화에서 `["해수욕"]` 으로 채워둔다
- **온천 87곳 전부 `swimsuit`(커플이 수영복 입고 함께 들어갈 수 있는지)이 확정**됐다.
  이 앱에서 온천의 가장 중요한 값이다
- **같은 뜻인데 이름이 다른 항목**은 SPOTS 아래 정규화에서 맞춘다 — sig/menu→signature,
  hours→open(카페·맛집), facility→fac(캠핑), price→fee, stopBy→stop, stay→spend,
  level_note→lvTxt, when_hours→hours. 차수마다 조사팀이 다른 이름을 써서 화면에
  안 나오던 값이 600건 있었다
- **등산 난이도는 easy/mid/hard 세 값만 쓴다.** 조사팀이 '초급·중급·초중급' 같은 한글로
  보내오면 `LEVEL_KO` 가 자동으로 바꿔준다. 한글이 그대로 들어가면 표기가 제각각이 된다
- 조사팀은 "확인 못 함" 같은 **메모를 값 안에 적어 보낸다.** `pull-patch.py` 가
  괄호 안 메모는 떼고 문장 전체가 메모면 그 칸을 버린다. tip 은 일부러 넣은 경고문이라 손대지 않는다

- ⚠️ **인천 행정구역 개편(2026.7)** — 중구→영종구·제물포구, 서구→서해구·검단구.
  카카오에 이미 반영돼 있으니 인천 스팟을 새로 넣을 땐 새 구 이름을 쓸 것
...
### 빈 시군구 세는 법 (지도에 휑한 데를 찾는 법)
배포된 사이트 콘솔에서 돌린다. `SGG_SHAPES[i].b` 가 시군구 경계상자([x0,y0,x1,y1], SVG 좌표)다.

```js
const ll=(lat,lng)=>({x:(lng-125.8)*70+10, y:(38.8-lat)*69+10});
const A=allSpots().filter(s=>s.lat!=null).map(s=>ll(s.lat,s.lng));
SGG_SHAPES.map(g=>{const[x0,y0,x1,y1]=g.b;
  return {n:g.n, k:A.filter(s=>s.x>=x0&&s.x<=x1&&s.y>=y0&&s.y<=y1).length};})
 .filter(r=>r.k<=2).sort((a,b)=>a.k-b.k);
```

⚠️ **경계상자라서 이웃 시군 스팟이 딸려 잡힌다.** 예천 칸에 안동 하회마을이, 성주 칸에 합천
홍류동계곡이, 광양 칸에 순천 드라마촬영장이 잡혔다. 숫자가 1~2로 나오면 그 안의 스팟 이름을
꼭 확인할 것 — 이름이 다른 시군 것이면 실제로는 0곳이다.

### 데이터 품질 점검 (tools/audit.js)
`node tools/audit.js` → `tools/audit.txt`. 아래를 한 번에 훑는다.
- 카드에 표시되지 않는데 값만 들어 있는 고아 필드 (SPOT_ROWS·SPOT_FLAGS 양쪽과 대조)
- 한 카드 안에서 같은 아이콘 라벨이 두 번 나오는지
- 예약 URL에 `&amp;` 나 한글 도메인이 섞였는지
- km ↔ kmTxt, pk ↔ pkTxt 가 서로 어긋나는지
- 해변인데 acts4 가 없는지, 등산 level 이 easy/mid/hard 밖의 값인지

여기에 더해 **좌표 충돌**도 주기적으로 본다. 소수점 5자리까지 같은 좌표에 같은 분류가 둘 이상
있으면 지도에서 서로를 가린다(여의도한강공원 한 점에 축제 2·캠핑 1·드라이브 1이 겹쳐 있었다).
분류가 서로 다른 겹침은 정상이다 — 같은 장소를 숙박·온천·스키로 나눠 넣은 것들이다.

### 예약 링크 살아 있는지 확인
```bash
node -e "…SPOTS+RUN_SPOTS 에서 book 의 http URL 만 뽑아 tools/_urls.txt 로…"
tr -d '
' < tools/_u.txt | xargs -P 4 -I{} bash tools/chk1.sh "{}" > tools/_urlchk2.txt
```
`chk1.sh` 는 curl 로 상태코드만 찍는다. 200이 아닌 것만 손보면 된다.
**403은 대개 봇 차단**(하얏트·메리어트·반얀트리)이라 실제로는 살아 있다. 404·000이 진짜 문제다.
지자체 관광 페이지는 `/tour/` 로 끝나면 404가 잦고 `/tour/index.do` 로 열리는 경우가 많다.
...
- **폐업 확인으로 뺀 곳 (26-08-27)**: 지리산온천랜드(구례) — 2020년 휴업 후 재개장 정황 없음.
  문화일보·시사저널·조선일보 세 곳이 일치. `pull-spots.py` REJECT 에 넣어 막아뒀다
- **휴업·부분휴장이라 tip 에 경고를 박아 둔 곳**: 진안홍삼스파(임시 휴업 표시, 재개장 미확정),
  스파밸리(2026.8.31~12.31 계절 휴장), 석모도 미네랄온천(노천탕 미운영·실내탕만),
  서울 밤도깨비야시장(공식 사이트 폐쇄, 최근 운영 여부 불명)
...
### ⚠️ 절대 넣으면 안 되는 주소
- `bonghwafestival.com` — 봉화은어축제 옛 공식 도메인. **만료 후 탈취돼 포르투갈어 도박 사이트**가 됐다.
  봉화은어축제는 봉화축제관광재단 `bhftf.or.kr` 이 정상 운영 중이다
- `mirandahotel.com` — 이천 미란다호텔 옛 공식 도메인. 만료 후 무관한 사이트로 넘어갔는데
  검색엔진에는 아직 공식으로 남아 있다
- 한글 도메인(`http://축제이름.com` 꼴)은 punycode 로 안 열리는 경우가 있어 넣지 않는다.
  영광 불갑산 상사화축제가 이 경우였다

지자체 축제는 **전용 도메인이 해마다 만료된다.** 안 열리면 주최 지자체 관광 페이지로 대체하는 게
오래 간다. https 가 죽고 http 만 사는 곳도 있다(진해군항제 `jgfestival.or.kr`, 탐라문화제
`tamnafestival.kr`, 청양 알프스마을 `alpsvill.com`) — 이럴 땐 http 로 넣어야 열린다.

⚠️ **링크 점검 스크립트에 CRLF 함정이 있다.** 파이썬으로 URL 목록을 파일에 쓰면 윈도우에서
줄끝이 `
` 이 되고, `xargs` 가 `
` 까지 URL 에 붙여 넘겨 **전부 000(연결 실패)로 나온다.**
살아 있는 주소를 죽었다고 오판하기 딱 좋으니 `tr -d '
'` 를 한 번 통과시킬 것.

### 죽은 예약 링크 교정 (tools/url-fix.json)
`pull-patch.py` 는 세션 기록 전체를 다시 긁으므로, 손으로 고친 값이 **다음 차수에
되돌아온다.** 도메인이 사라져 눌러도 안 열리던 주소를 고친 것은
`tools/url-fix.json` 에 적어 두고 **apply-patch 를 돌린 뒤 반드시 한 번 더** 적용할 것.

```bash
python tools/pull-patch.py <세션.jsonl> tools/_patch.json
python tools/apply-patch.py tools/_patch.json
python tools/apply-patch.py tools/url-fix.json   # ← 이걸 빼먹으면 죽은 링크가 되살아난다
```

예약 링크를 넣은 뒤에는 **도메인이 실제로 사는지 DNS로 확인**할 것. 조사팀이 준
주소 중 실제로 소멸한 것이 여럿 있었다(inspirekorea.com, scgcamp.com,

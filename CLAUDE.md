...
### 현재 수량 (2026-08-27)
숙박 144 / 등산 133 / 카페 128 / 맛집 122 / 전시·구경 106 / 캠핑 96 / 쇼핑·시장 95 /
축제 88 / 계곡 86 / 온천 86 / 드라이브 85 / 해변 81 / 공연 80 / 스키·썰매 52
= **스팟 1,382곳** + 러닝 188곳 = **1,570곳**, 좌표 100%
- **예약 정보 731곳, 그중 공식 예약 URL 281곳.** 숙박 144/144, 전시 106/106,
  맛집 121/122, 공연 80/80, 캠핑 92/96, 스키 52/52 는 사실상 전수 완료
- **해변은 `acts4` 배열로 4가지 활동을 겹쳐 표시**한다(해수욕 68·서핑 16·프리다이빙 4·
  스쿠버 5·해당없음 13). 해변을 고르면 세부 칩이 이 4가지로 바뀌고, 한 해변이 여러 칸에
  동시에 들어간다. 서핑·다이빙이 안 되는 곳은 그 줄을 감춘다(BEACH_ACTS 참고)
- **온천 86곳 전부 `swimsuit`(커플이 수영복 입고 함께 들어갈 수 있는지)이 확정**됐다
  (가능 51 / 불가 35). 이 앱에서 온천의 가장 중요한 값이다
- **같은 뜻인데 이름이 다른 항목**은 SPOTS 아래 정규화에서 맞춘다 — sig/menu→signature,
  ...
  차수마다 조사팀이 다른 이름을 써서 화면에 안 나오던 값이 600건 있었다
...
- **같은 뜻인데 이름이 다른 항목**은 SPOTS 아래 정규화에서 맞춘다 — sig/menu→signature,
  hours→open(카페·맛집), facility→fac(캠핑), price→fee, stopBy→stop, stay→spend.
  차수마다 조사팀이 다른 이름을 써서 화면에 안 나오던 값이 600건 있었다
- 온천은 `swimsuit` 이 커플에게 가장 중요한 값 (수영복 입고 함께 들어갈 수 있는지)

...
REJECT = {
    ...
    ("spa", "변산온천"), ("spa", "변산온천리조텔"),
    # 팀마다 말이 달라 카카오로 판정 - 둘 다 카카오에 실체가 없어 제외
    ("spa", "드래곤힐스파 (용산)"), ("spa", "실로암불가마사우나 (서울역)"),
    # 조사팀이 업소를 특정 못 하고 '분류형 참고 항목'으로 넣은 것 — 실체가 없음
    ("stay", "북촌빈관 by 락고재(전주 대안: 한옥마을 스테이)"),
    ("food", "우리옥"),   # 강화. 카카오에 기록 없음 - 좌표를 추측할 수 없어 제외
    # 이천 미란다 스파플러스: 한 팀은 도메인 만료, 다른 팀은 운영 중이라 함.
    # 카카오에도 기록이 없어(호텔 본관만 있음) 실체 확인 불가 - 제외
    ("spa", "미란다 스파플러스"),
    ("camp", "서귀포 카라반"),   # 국군복지단 시설 — 일반인은 예약 자체가 안 됨
    # 폐업·업종전환 확인 (26-08-27)
    ("cafe", "새빌"),          # 제주 새별오름 — 같은 자리에 베이크하우스 운포피우
    ("cafe", "신기산업"),      # 부산 영도 — 같은 자리에 미피카페 부산점
    # 같은 곳이 이름만 달리해 두 번 들어온 것 — 정보 많은 쪽으로 이미 합쳤다
    ("stay", "곤지암리조트"), ("valley", "물한계곡"), ("hike", "석성산"),
    ("hike", "호룡곡산~국사봉(무의도)"), ("stay", "RYSE 오토그래프 컬렉션"),
    ("stay", "아난티 앳 부산 코브"), ("food", "용성통닭 본점"),
    ("fest", "얼음나라 화천 산천어축제"),
    # 카카오에 상호 기록이 없어 좌표를 확인할 수 없다 (조사팀도 conf:low 로 검증 권고)
    ("shop", "계룡 엄사중앙시장"),
    # 카카오에 업소 기록이 아예 없어 좌표를 확인할 수 없는 곳 (주소만 존재하거나 그마저도 없음)
    ("cafe", "센터커피 서울숲점"),
    ("spa", "실로암사우나 (서울역·중림동)"),
    # 같은 축제가 띄어쓰기만 달리해 두 번 들어온 것 — '익산 서동축제'로 합쳤다
    ("fest", "익산서동축제"),
    # 2020년 휴업 후 재개장 정황이 없다 (문화일보·시사저널·조선일보 교차 확인) — 사실상 폐업
    ("spa", "지리산온천랜드"),
}
REJECT_CAT = {"resort"}
